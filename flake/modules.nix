...
let
      ...
      lib.mapAttrsToList (name: type:
        if type == "directory" then
          ...
          in [
            ...
            {
              name = name;
              value = { imports = (map (x: x.value) children); };
            }
          ]
        else {
  ...
  in path: flatten (lib.fix collect path);
in {
  options = {
    modules = {
      nixosRoot = mkOption {
  ...
  config = {
    flake.nixosModules =
      mkIf (opts.nixosRoot != null) (build_modules opts.nixosRoot);

    flake.homeModules =
      mkIf (opts.nixosRoot != null) (build_modules opts.homeRoot);
  };
}
