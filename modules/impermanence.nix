  ...
  personal.checks.impermanence = {
    modules = [
      ...
      (
        { pkgs, config, lib, ... }:
        {
          ...
          boot.initrd.systemd.services."btrfs-subvols" = let
    ...
    script = ''
      ...
      machine.succeed(
          ...
          "touch /var/lib/nixos/persistent"
      )
      machine.succeed("test -f /persistent/var/lib/nixos/persistent")
      ...
      machine.succeed("test -f /var/lib/nixos/persistent")
